    ...
    public class CarparkGateway : DataGateway<Carpark>
    {
        ...
        public void importCSV(int i)
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                if (values[0] != string.Empty && values[1] != string.Empty && values[2] != string.Empty && values[3] != string.Empty && values[4] != string.Empty && values[5] != string.Empty && values[6] != string.Empty && values[7] != string.Empty && values[8] != string.Empty && values[9] != string.Empty)
                {
                    try
                    {
                        db.Database.ExecuteSqlCommand("INSERT INTO carpark (carparkNo, address, x_coord, y_coord, carparkType, typeOfparking, shortTermparking, freeParking, nightParking, parkAndrideScheme, adhocParking) VALUES ('" + i+"-"+values[0].Replace("\"", string.Empty).Trim() + "', '" + i + "-" + values[1].Replace("\"", string.Empty).Trim() + "', " + values[2].Replace("\"", string.Empty).Trim() + ", " + values[3].Replace("\"", string.Empty).Trim() + ", '" + values[4].Replace("\"", string.Empty).Trim() + "', '" + values[5].Replace("\"", string.Empty).Trim() + "','" + values[6].Replace("\"", string.Empty).Trim() + "', '" + values[7].Replace("\"", string.Empty).Trim() + "', '" + values[8].Replace("\"", string.Empty).Trim() + "','" + values[9].Replace("\"", string.Empty).Trim() + "', '')");
                    }
                    catch (Exception ex)
                    {
        ...
        public IEnumerable<Carpark> GetTopFiveBookmarks()
        {
            ...
            //for (int i = 1; i <= 111448; i++)
            //{
            //    if (i%2==0)
            //    {
            //        db.Database.ExecuteSqlCommand("UPDATE parkwhere.user SET status = 'active' WHERE UserId='" + i + "'");
            //    }else
            //    {
            //        db.Database.ExecuteSqlCommand("UPDATE parkwhere.user SET Status = 'inactive' WHERE UserId='" + i + "'");
            //    }
